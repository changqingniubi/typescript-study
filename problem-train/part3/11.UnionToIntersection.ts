 ...
 * @LastEditTime: 2021-12-08 15:54:07
...
type UnionToIntersection2<T>=(T extends any?(args:T)=>any:never) extends (args: infer R)=>any?R:never

type A = UnionToIntersection2<{a: string} | {b: string} | {c: string}> 
...
*/
type AA = UnionToIntersection<{ a: string } | { b: string } | { c: string }> // {a: string} & {b: string} & {c: string}
